 ...
 * topic模式
 * 消费者
 * routingKey只要符合info.#的匹配规则 数据会被推送到topic_info关联的队列中
 *
 * @author sobann
 */
public class TopicInfoAllConsumer extends ExchangeConnectionSupport {
    ...
    public static void main(String[] args) throws Exception {
        ...
        // 声明info队列
        channel.queueDeclare("topic_info", false, false, false, null);
        channel.queueBind("topic_info", ExchangeConnectionSupport.TOPIC_EXCHANGE_NAME, "info.#");

        // 消费者
        Consumer consumer = createConsumer(channel);

        // 消费
        channel.basicConsume("topic_info", true, consumer);
